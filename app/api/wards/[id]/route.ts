...
import { getWard, softDeleteWard, updateWard } from "@/lib/services/rooms";
...
export async function GET(_request: Request, { params }: RouteContext) {
  try {
    ...
    const data = await getWard(id, hospitalId);
    if (!data) return apiError("Ward not found", 404);
...
export async function PATCH(request: Request, { params }: RouteContext) {
  try {
    ...
    const result = await updateWard(id, hospitalId, parsed.data, profile.id);
...
export async function DELETE(_request: Request, { params }: RouteContext) {
  try {
    const { id } = await params;
    const { hospitalId, profile } = await requireServiceContext("admissions:manage");
    const result = await softDeleteWard(id, hospitalId, profile.id);
    return apiServiceResult(result);
  } catch (error) {
    const failure = toApiError(error);
    return apiError(failure.error, failure.status, failure.details);
