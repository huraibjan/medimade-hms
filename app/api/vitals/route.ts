...
import { addVitals, listVitals } from "@/lib/services/vitals";
import { requireServiceContext, toApiError } from "@/lib/services/service-context";
import { vitalsSchema } from "@/lib/validations/clinical";
...
export async function GET(request: Request) {
  try {
    ...
    const data = await listVitals(patientId, hospitalId);
...
export async function POST(request: Request) {
  try {
    const { hospitalId, profile } = await requireServiceContext("vitals:manage");
    ...
    const result = await addVitals(hospitalId, parsed.data, profile.id);
